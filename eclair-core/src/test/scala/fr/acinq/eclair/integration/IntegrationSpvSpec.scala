...
class IntegrationSpvSpec extends TestKit(ActorSystem("test")) with FunSuiteLike with BeforeAndAfterAll with Logging {
  ...
  override def beforeAll(): Unit = {
    ...
    Files.copy(classOf[IntegrationSpvSpec].getResourceAsStream("/integration/bitcoin.conf"), new File(PATH_BITCOIND_DATADIR.toString, "bitcoin.conf").toPath)
...
//    instantiateEclairNode("F4", ConfigFactory.parseMap(Map("eclair.node-alias" -> "F4", "eclair.server.port" -> 29738, "eclair.api.port" -> 28088, "eclair.payment-handler" -> "noop")).withFallback(commonConfig))
  ...
  def sendFunds(node: Kit) = {
    ...
    sender.send(bitcoincli, BitcoinReq("sendtoaddress", address, 1))
    sender.expectMsgType[JValue](100000 seconds)
  }
